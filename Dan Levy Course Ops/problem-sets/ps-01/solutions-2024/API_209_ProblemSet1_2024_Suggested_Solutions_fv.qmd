...
::: answer-box
*The opposition counted 10,887,262 valid votes.*
:::
...
#percent won by Gonzalez
votes_EG <- sum(venezuela$EG)/ValidVotesActas

#percent won by Maduro
votes_NM <- sum(venezuela$NM)/ValidVotesActas

#percent won by Other
votes_Other <- sum(venezuela$Other)/ValidVotesActas
...
::: answer-box
The percent of votes won by Gonzalez is: `r round(votes_EG*100,2)`%

The percent of votes won by Maduro is: `r round(votes_NM*100,2)`%.

The percent of votes won by Other is: `r round(votes_Other*100,2)`%
:::
...
5.  Now compute the difference in votes between Gonzalez and Maduro in percentage points for each state in Venezuela. Call this indicator “gap.” For example, if in a state Gonzalez won 62% of the votes and Maduro won 32% of the votes, this gap indicator would be 30 percentage points. If, on the other hand, Gonzalez won 32% of the votes and Maduro won 62%, the gap would be -30 percentage points. Please answer the following questions:
    ...
    b.  Describe in a short paragraph your findings from the question just above.
        ...
        2.  Ask [PingPong](https://pingpong.hks.harvard.edu/group/28) -an HKS homegrown AI tool that taps into ChatGPT- for help in improving your paragraph. Copy and paste your prompt and resulting AI advice here. If for some reason this AI tool does not work for you, use one that is available to you (ChatGPT, Claude, etc.)
            ...
            *Response: From the analyzed data, it is evident that the Venezuelan opposition party has a compelling basis for their accusations of election fraud against the government. The data indicates that for 83% of the votes collected by the opposition, González maintains a significant lead over Maduro across all 24 states. Notably, González's lead exceeds 20 percentage points in all but two states, Sucre and Delta Amacuro. Furthermore, in states such as Táchira, Mérida, and Barinas, González's lead surpasses 50 percentage points. This consistent and substantial advantage for González throughout the country strongly supports the opposition's claim of electoral fraud, as it suggests a nationwide pattern of disproportionate vote distribution.*

            *The entire conversation with the AI tool: https://pingpong.hks.harvard.edu/group/28/thread/11359*
            :::
    ...
    ::: answer-box
    The percentage of voting centers in Barinas that Gonzalez won is: `r round(per_barinas,2)*100`%.
    ...
    d.  Given recent election history, Maduro was predicted to win in rural areas.
        ...
        ::: answer-box
        One possible approach is to look at overall urban/rural divide in Venezuela. According to the World Bank, an estimated 3,334,307/28,838,499, or 11.56%, of the total population were rural in 2023. Scaling that to voters, our goal is to identify which 11.56% of voters to include in a "rural" estimate. Online density maps from EarthData at Colombia are old (from 2000), but they suggest low population densities in four SouthEastern states in particular: Amazonas, Bolivar, Delta Amacuro and Apure. Guarico has some density, but relatively less than remaining states acording to Columbia's density map.
        :::
        ...
        #That gets us to 7.3%, leaving roughly 4% left to hit the estimates for the rural population. Adding Guarico...
        ...
        ::: answer-box
        For every 100 rural voters in Venezuela, this analysis estimates that `r round(votes_EG*100)` voted for Gonzalez, whereas only `r round(votes_NM*100)` voted for Maduro, with the remainder voting for an alternate candidate, implying that Gonzalez, not Maduro, was winning among rural voters.
    ...
    *I would have estimated that the probability that Maduro would win the election was very low, maybe 20 or 30 percent. This is due to the economic, political and migration crises that Venezuela is going through with the current president Maduro, and the fact that almost all polls conducted prior to the election gave Gonzalez a large lead.*
    ...
    7.  Now let’s look at the numbers the CNE reported on election night (below). When you calculate the exact percentages (see last column of table below), these numbers look suspicious. But sometimes numbers that are suspicious are the result of natural statistical fluctuations. We will examine whether this is one of these cases.
        ...
        ::: answer-box
        *Let B = Results (the results as reported by the opposition) Let A = MaduroWin (N. Maduro gets the most votes)*
...
6.  After reading Gelman’s blog, another analyst says “This is way too complicated; I have an easier way to prove that there was fraud. How odd is it that the last digit of Maduro’s vote count ends in 2 and Gonzalez ends in 8, which happens to add up to a round number (10)? Not very likely. Prob (Maduro ends in 2)=1/10 and Prob(Gonzalez ends in 8)=1/10. Hence Prob(Maduro ends in 2 **and** Gonzalez ends in 8)=(1/10)\*(1/10)=1/100. Basically just a 1% chance that these results could have arisen by chance. Very unlikely results. Pretty clear evidence of fraud.”
    ...
    *Both theories involve probabilities and low-probability events. Both are evaluating the likelihood of obtaining the patterns of numbers that were reported in the Venezuelan election. Both find that the combination of numbers found is unlikely, and therefore, conclude that there was probably fraud in the elections. However, it is different since the new analyst is analyzing the likelihood of the last digits adding up to 10, and the first analyst is evaluating the likelihood of getting the rounded percentages that were reported in the elections. The likelihoods are also orders-of-magnitude different.*
    ...
    b.  Do you think this reasoning is sound? If so, why. If not, why not?
        ...
        ::: answer-box
        *I do not think the evidence presented is sound. First, the analyst incorrectly calculated the probability of the last digits adding up to 10. If that's the probability the analyst was evaluating, they should have included the digit pairs 1-9, 2-8, 3-7, 4-6, 5-5, 6-4, 7-3, 8-2 and 9-1 in their calculation which would signficantly increase the odds. Second, even if the analyst was commenting on the specific 2-8 pair probability and that low likelihood, many other patterns of last digits would be just as unlikely. For example, under this logic, the probability of Maduro ending in 3 and Gonzalez ending in 8 would also be 1 percent.*
        :::

7.  \[Optional\] Draft an X post (formerly known as Tweet, max: 280 characters) with one of your key findings from this problem set. Make sure you include some visual aid (a table, graph, map, etc).\* If you would like to share with your classmates, please post on our Slack workspace using the #r-tips channel If you would like to share with the world, please post on X (i.e., tweet) using #API209.
...
::: answer-box
Answer: See your notes from class.
...
$$
\begin{align}
P(\text{Enrolled}| \text{Not poor}) &= \frac{\text{\# enrolled and not poor}}{\text{\# not poor}} \\
 &=\frac{1,234,416}{1,872,313} \\
 &= 0.659
\end{align}
$$ Option 2\
Leakage =
...
$$
\begin{align}
P(\text{Not Enrolled}| \text{poor}) &= \frac{\text{\# poor and not enrolled}}{\text{\# poor}} \\
 &=\frac{1,234,416}{1,872,313} \\
 &= 0.659
\end{align}
$$
